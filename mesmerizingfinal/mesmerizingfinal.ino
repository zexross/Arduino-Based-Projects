...
int sensor_1,sensor_2,sensor_3,sensor_4,sensor_5,sensor_6,no_of_active_sensors,Switch=0;
int index = 0, data_index = 0;
char sensor_select = 'f',motor_state = 'f',state;
char learned_path[500];
...
void pid_calculation(){
  int left_extreme, right_extreme,SPEED=50;
  p = kp*error;
...
void learned_pathexpo(){
  ...
  if(no_of_active_sensors==1 || no_of_active_sensors==2){
  //Serial.println("ok2");
  if(sensor_1==0 && sensor_2==0 && sensor_3==0 && sensor_4==0 && sensor_5==0 && sensor_6==1){
  error = 6;
  pid_calculation();
 ...
 else if(no_of_active_sensors==4 || no_of_active_sensors==6){
  char recorded_state = learned_path[data_index+1];
  switch(recorded_state){
    case 'R':
    turn_left();
    delay(755);
    data_index++;
